...
import useMonthGames from "../hooks/useMonthGames.js";
...
function GamesByMonth() {
  ...
  const monthNo = useSelector((state) => state.month.monthNo);
  const {
    data: monthGamesData,
    isLoading,
    hasNextPage,
    fetchNextPage,
    isFetchingNextPage,
  } = useMonthGames({ firstSelectValue, platform, monthNo });

  const dataLength =
    monthGamesData?.pages.reduce(
  ...
  return (
    <section className="flex-col px-4 tablet:mt-6 tablet:flex tablet:w-full tablet:px-0">
      ...
      >
        {isLoading && <Spinner />}
        {monthGamesData?.pages.map((page, i) => (
          <div
            key={i}
